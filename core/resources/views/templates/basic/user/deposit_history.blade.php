                ...
                <tbody>
                    @forelse($deposits as $deposit)
                        <tr>
                            <td>
                                <div>
                                    <span class="fw-bold" style="color:#0f2421;">{{ __($deposit->gateway?->name) }}</span>
                                    <br>
                                    <small style="color:#7a9e9a; font-size:0.78rem;">{{ $deposit->trx }}</small>
                                </div>
                            </td>
                            ...
                            <td>
                                <div>
                                    <span style="font-weight:700; color:#0f2421;">{{ gs('cur_sym') }}{{ showAmount($deposit->amount) }}</span>
                                    +<span class="text--danger" title="@lang('charge')">{{ showAmount($deposit->charge) }}</span>
                                    <br>
                                    <strong style="font-size:0.8rem;" title="@lang('Amount with charge')">
                                        {{ showAmount($deposit->amount + $deposit->charge) }} {{ __(gs('cur_text')) }}
                                    </strong>
                                </div>
                            </td>
                            <td>
                                <div style="font-size:0.855rem;">
                            ...
                            <td>
                                @if ($deposit->method_code >= 1000)
                                    ...
                                    <a href="javascript:void(0)" class="badge badge--icon badge--fill-base detailBtn"
                                       data-bs-toggle="tooltip" data-bs-position="top" title="@lang('View')"
                                ...
                                @else
                                    <a href="javascript:void(0)" class="badge badge--icon badge--fill-base disabled" title="@lang('View')">
                                        <i class="fas fa-desktop"></i>
                                    </a>
                                @endif
                            </td>
...
(function($) {
    ...
    $('.detailBtn').on('click', function() {
        ...
        if (userData) {
            userData.forEach(function(element) {
                if (element.type != 'file') {
                    html += `<li class="list-group-item d-flex justify-content-between align-items-center" style="font-size:0.855rem;">
        ...
        if ($(this).data('admin_feedback') != undefined) {
            adminFeedback = `<div class="mt-3 p-3" style="background:rgba(239,68,68,0.07); border-radius:10px; font-size:0.855rem;">
